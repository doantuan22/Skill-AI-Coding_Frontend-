...
|---|---|---|---|
| [package-manifest.schema.json](package-manifest.schema.json) | `PACKAGE-MANIFEST.json`, embedded in every artifact and published next to it: identity, provenance and per-file sha256 | `packaging/build.py` | `packaging/verify.py` (V1) |
...
The plugin manifest schema is at [plugin.schema.json](plugin.schema.json). Validation uses the JSON Schema subset implemented in `packaging/artifact.py` (`validate_schema`), so no third-party validator is needed.
