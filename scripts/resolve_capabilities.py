...
phase-2/capability-resolver/README.md) and never installs or fetches anything.
...
import knowledge_lib as K

INTENSITY_LEVEL = {"low": 1, "medium": 2, "high": 3}
DENSITY_LEVEL = {"low": 1, "medium": 2, "high": 3}
COST_POINTS = {"none": 0, "low": 1, "medium": 2, "high": 4, "very-high": 6}
...
def resolve_layouts(entries: dict[str, dict], p: dict, primary: dict, secondary: dict | None, screens: list[dict]) -> dict:
    ...
    for layout in _of_kind(entries, "layout"):
        ...
        if layout["default_tell"] == "true" and primary["id"] not in p["explicit_styles"]:
            score -= 2; reasons.append("default-tell penalty (anti-homogenization)")
        if score <= 0:
...
def resolve_motion(entries: dict[str, dict], p: dict, primary: dict, secondary: dict | None,
    ...
    for source in sources:
        for mid in source:
            if mid not in order:
                order.append(mid)
    selected, rejected, high_taken = [], [], None
    for mid in order:
        m = entries[mid]
        ...
        if m["intensity"] == "high":
            if high_taken:
                rejected.append({"id": mid, "reason": f"motion budget allows one HIGH region ({high_taken} already selected)"}); continue
            high_taken = mid
...
def resolve_effects(entries: dict[str, dict], p: dict, primary: dict, secondary: dict | None, tell_budget: list[str]) -> dict:
    ...
    for eid in candidates:
        ...
        if e["default_tell"] == "true" and primary["id"] not in p["explicit_styles"]:
            if tell_budget:
...
def resolve(raw_profile: dict, entries: dict[str, dict] | None = None) -> dict:
    if entries is None:
        ...
        if errors:
            raise ProfileError("knowledge base failed to load: " + "; ".join(errors[:3]))
    ...
    if secondary and entries[secondary["id"]]["default_tell"] == "true" and secondary["id"] not in p["explicit_styles"]:
    ...
    for item in layouts["selected"]:
        if entries[item["id"]]["default_tell"] == "true" and primary["id"] not in p["explicit_styles"]:
            if tell_budget:
