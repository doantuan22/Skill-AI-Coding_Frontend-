...
Public entry: ``analyze(project, manifests, visual_intensity)`` (exposed as ``uiux.api.analyze_design_quality``).
...
def evaluate(s: dict, rt: dict | None, visual_intensity: int) -> dict:
    ...
    # E72 Effect Overuse
    # Effect kinds weighted by the shared cost model (backdrop/canvas high, blur/large shadows/loops medium, rest low).
    points = (COST["high"] if s["backdrop_filters"] else 0) + (COST["high"] if s["backdrop_filters"] > 3 else 0) \
        + (COST["medium"] if s["blur_filters"] else 0) + (COST["medium"] if s["large_shadows"] else 0) \
        + (COST["low"] if s["gradients"] else 0) + (COST["low"] if s["gradients"] > 8 else 0) \
        + (COST["low"] if s["blend_modes"] else 0) + (COST["high"] if s["canvas_or_webgl"] else 0) \
        + (COST["medium"] if s["infinite_animations"] > 2 else 0)
...
def analyze(project: Path | str, manifests: list[Path | str] | tuple = (), visual_intensity: int = 3) -> dict:
    """Collect static signals (+ optional runtime probes) and evaluate the E65-E80 heuristics."""
    project = Path(project).resolve()
    if not project.is_dir():
        raise ValueError("project directory not found")
    if visual_intensity not in EFFECT_BUDGET:
        raise ValueError("visual_intensity must be 1..5")
    signals, count = collect(project)
    runtime = runtime_summary([Path(m) for m in manifests])
    return {
        "schema_version": 1, "project": str(project), "files_scanned": count, "visual_intensity": visual_intensity,
        "signals": signals, "runtime": runtime, "evals": evaluate(signals, runtime, visual_intensity),
...
def main(argv: list[str]) -> int:
    ...
    try:
        report = analyze(args.project, args.manifest, args.visual_intensity)
