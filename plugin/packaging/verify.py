...
def check_whitelist(v: Verifier, root: Path, metadata_file: str, ignore_caches: bool = False) -> list[str]:
    result = v.run([str(root / "plugin/packaging/package_files.py"), "--list"])
    try:
...
def check_identity(root: Path, manifest: dict, core: dict, archive_base: str | None, require_release: bool) -> list[str]:
    ...
    plugin = json.loads((root / "plugin/manifest/plugin.json").read_text(encoding="utf-8"))
...
def check_generic_adapter(v: Verifier, root: Path) -> list[str]:
    adapter = root / "plugin/adapters/generic/adapter.py"
    problems = []
    ...
    for metadata in sorted((root / "plugin/adapters").glob("*/adapter.json")):
