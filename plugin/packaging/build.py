...
def select(root: Path) -> list[dict]:
    code, output = run_tree_tool(root, "plugin/packaging/package_files.py", "--list")
    try:
...
def preflight(root: Path) -> dict:
    code, output = run_tree_tool(root, "scripts/knowledge_lib.py", "check")
    if code != 0:
...
def package_manifest(root: Path, entries: list[dict], ident: dict, source: dict, rules: dict, dev: bool
    ...
    for entry in entries:
        ...
        data = artifact.normalize(rel, (root / rel).read_bytes(), binary)
    ...
    rules_bytes = b"".join(artifact.normalize(p, (root / p).read_bytes(), binary)
                           for p in (artifact.RULES_RELATIVE, "uiux/core/layers.json"))
...
def build(repo: Path = DEFAULT_REPO, commit: str = "HEAD", dev: bool = False, out: Path | None = None,
    ...
    with tempfile.TemporaryDirectory(prefix="uiux-build-") as temporary:
        ...
        problems = artifact.validate_schema(manifest, json.loads(
            (root / "plugin/schemas/package-manifest.schema.json").read_text(encoding="utf-8")))
        if problems:
